...
const PersonalInfoForm = ({data, onChange, removeBackground, setRemoveBackground}) => {
  ...
  return (
    <div>
      ...
      <p className='text-sm text-gray-600'>Get started with the personal details.</p>
      <div className='flex items-center gap-2'>
        <label>
            {data.image ? (
                <img src={typeof data.image === 'string' ? data.image : URL.createObjectURL(data.image)} alt="user-image" className='size-16 rounded-full object-cover mt-5 ring ring-slate-300 hover:opacity-80' />
            ) : (
                <div className='inline-flex gap-2 mt-5 items-center justify-center text-slate-600 hover:text-slate-700 cursor-pointer'>
                    <User className='size-10 p-2.5 border rounded-full' />
                    Upload user image
                </div>
            )}
            <input onChange={(e) => handleChange('image', e.target.files[0])} type="file" accept='image/*' className='hidden' />
        </label>
